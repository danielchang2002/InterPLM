...
#SBATCH --gpus=1
#SBATCH --mem=32GB
#SBATCH --output=logs/eval_esm2_8m-%A_%a.out
#SBATCH --error=logs/eval_esm2_8m-%A_%a.err
#SBATCH --array=0-7
#SBATCH --requeue
#SBATCH --signal=B:TERM@120

# Step 2 of 5: ESM2-8M layer 4 — embed + SAE encode per shard (GPU)
# Run after eval_setup.sh completes.
# After this completes, run: sbatch data_scripts/eval_esm2_8m_compare.sh
...
echo "=== ESM2-8M Layer $LAYER — Shard $SHARD ==="
...
embeddings_dict = embedder.extract_embeddings_with_boundaries(sequences, layer=$LAYER, batch_size=64)
