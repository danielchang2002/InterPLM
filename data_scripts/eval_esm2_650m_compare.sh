...
#SBATCH --mem=64GB
#SBATCH --output=logs/eval_esm2_650m_compare-%A_%a.out
#SBATCH --error=logs/eval_esm2_650m_compare-%A_%a.err
#SBATCH --array=0-15
#SBATCH --requeue

# Step 5: Compare cached SAE features to annotations (CPU, array per shard+eval_set)
# Array 0-7 = valid shards 0-3, Array 8-15 = test shards 4-7
# Run after eval_esm2_650m.sh completes.
# After this completes, run: sbatch data_scripts/eval_esm2_650m_f1.sh
...
echo "=== ESM2-650M Compare — $EVAL_SET shard $SHARD (task $TASK_ID) ==="
echo "Started: $(date)"
