...
#SBATCH --mem=64GB
#SBATCH --output=logs/eval_evo2_compare-%A_%a.out
#SBATCH --error=logs/eval_evo2_compare-%A_%a.err
#SBATCH --array=0-15
#SBATCH --requeue

# Step 3: Compare cached Evo2 SAE features to annotations (CPU, array per shard+eval_set).
# Array 0-7 = valid shards 0-7, Array 8-15 = test shards 0-7
# (Non-applicable shard/eval_set combos are skipped automatically.)
#
# Usage: sbatch data_scripts/eval_evo2_compare.sh <model> <strand>
#   model:  7b or 40b
#   strand: fwd or revcomp
#
# Run after eval_evo2_cache.sh completes for the same model+strand.
# After all 4 configs complete, run: sbatch data_scripts/eval_evo2_f1.sh

set -e

MODEL="${1:?Usage: eval_evo2_compare.sh <model: 7b|40b> <strand: fwd|revcomp>}"
STRAND="${2:?Usage: eval_evo2_compare.sh <model: 7b|40b> <strand: fwd|revcomp>}"
...
FEATS_DIR="$INTERPLM_DIR/data/sae_features/${CONFIG}"
RESULTS_DIR="$INTERPLM_DIR/results/${CONFIG}"

echo "=== Evo2 ${CONFIG} Compare — $EVAL_SET shard $SHARD (task $TASK_ID) ==="
echo "Started: $(date)"
